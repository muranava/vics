...
public class UserTest extends WebApplicationTest {
    ...
    public void createsANewUser() throws Exception {
        ...
        CreateUserRequest request = new CreateUserRequest("email@email.com", "password", "password", Role.USER, wardIDs, constituencyIDs, true);
    ...
    @Test
    public void failsIfEmailInvalid() throws Exception {
        String endpoint = "/user";
        when(sessionService.extractUserFromPrincipal(any(Principal.class)))
                .thenReturn(Try.success(admin()));

        CreateUserRequest request = new CreateUserRequest("com", "pw", "pw", Role.USER, emptyList(), emptyList(), true);
    ...
    @Test
    public void failsIfUsernameAlreadyExists() throws Exception {
        String endpoint = "/user";
        when(sessionService.extractUserFromPrincipal(any(Principal.class)))
                .thenReturn(Try.success(admin()));

        CreateUserRequest request = new CreateUserRequest("me@admin.uk", "pw", "pw", Role.USER, emptyList(), emptyList(), true);
    ...
    @Test
    public void failsIfNonAdminAccount() throws Exception {
        String endpoint = "/user";
        when(sessionService.extractUserFromPrincipal(any(Principal.class)))
                .thenReturn(Try.success(covs()));

        CreateUserRequest request = new CreateUserRequest("mdde@admin.uk", "pw", "pw", Role.USER, emptyList(), emptyList(), true);
